...
try:
    ...
    from clients.storage.abstract_storage import StorageError
except ImportError:
    logger.error("未能导入 S3 存储模块，请确保项目结构正确")
    sys.exit(1)
...
try:
    ...
    USE_CONFIG = False
    logger.warning("未找到配置模块，将直接从环境变量读取配置")
...
class S3StorageTest(unittest.TestCase):
    ...
    def test_connection(self):
        ...
        try:
            ...
            self.assertTrue(result, "存储桶检查应返回 True")
            logger.info("S3 连接测试通过")
        except Exception as e:
            self.fail(f"S3 连接测试失败: {str(e)}")
    ...
    def test_upload_file(self):
        ...
        try:
            ...
            # 验证是否返回了 URL
            self.assertIsNotNone(url, "上传文件应返回 URL")
            ...
            if self.s3_storage.config.get('endpoint_url'):
                logger.debug(f"S3 终端节点: {self.s3_storage.config['endpoint_url']}")
    ...
    def test_upload_image(self):
        ...
        try:
            ...
            # 验证是否返回了 URL
            self.assertIsNotNone(url, "上传图片应返回 URL")
    ...
    def test_upload_directory(self):
        ...
        try:
            ...
            # 验证返回的字典中包含上传的文件
            self.assertGreater(len(result), 0, "上传目录应返回至少一个文件的 URL")
...
def diagnose_s3_config():
    ...
    if USE_CONFIG:
        ...
        env_vars = {
            ...
            'S3_ENDPOINT_URL': os.getenv('S3_ENDPOINT_URL', '(未设置，使用 AWS S3)'),
    ...
    try:
        ...
        if s3_storage.check_bucket_exists():
            ...
            logger.error("S3 连接测试失败：无法访问或创建存储桶")
            return False
    except Exception as e:
        logger.error(f"S3 连接测试异常: {str(e)}")
        return False
